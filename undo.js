...
// Helper function to determine the rank based on coins
function getRankByCoins(coins) {
  if (coins >= 20000) return "Proofer";
  if (coins >= 10000) return "Diamond";
  if (coins >= 5000) return "Gold";
  if (coins >= 1000) return "Silver";
  if (coins >= 500) return "Iron";
  if (coins >= 0) return "Wood";
}
...
app.post('/api/task/complete', async (req, res) => {
  ...
  if (!telegramId || !coins || coins <= 0) {
    return res.status(400).json({ error: "Valid telegramId and a positive number of coins are required." });
  }
  ...
  try {
    ...
    // Add bonus coins
    user.coins += coins;
    await user.save(); // Save user with updated coin balance

    res.json({ 
      message: `Successfully completed the task! You have been awarded ${coins} coins.`,
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    const users = await User.find().sort({ coins: -1 }).limit(100); // Top 100 users by coins

    const leaderboard = users.map(user => ({
      name: user.name,
      coins: user.coins,
      rankTitle: getRankByCoins(user.coins), // Include rank title
    }));
        ...
        `Welcome, ${newUser.name}! to the $ProofCoin Farming Bot! 🌟\n\nUse the buttons below to start farming and earning $ProofCoins. You have ${newUser.coins || 0} coins.`,
        {
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: 'Website', url: 'https://topfoundation.vercel.app' }],
            ],
          },
        }
      );
